...
const PromoPage = () => {
    ...
    const [idPromo, setIdPromo] = useState<number>(0);
    ...
    const addPromo = async () => {
        ...
        try {
            const data = await postResource("promos", datapromo);
            if (data) {
                onCloseAdd()
                location.reload();
            } else {
    ...
    const openModalEdit = (item: PromoItem) => {
        setIdPromo(item.id);
        setName(item.name);
    ...
    const editPromo = async () => {
        ...
        try {
            const data = await updateResource(`promos/${idPromo}`, datapromo);
            if (data) {
                onCloseEdit()
                location.reload();
            } else {
    ...
    const deleteData = async () => {
        try {
            await deleteResource(`promos/${idPromo}`);
            onCloseDelete();
            location.reload();
        } catch (error) {
            console.log("Error fetching menus:", error);
    ...
    return (
        <div className="min-h-screen bg-[#f2f2f2] flex flex-col gap-1 text-black">
            ...
            <div className="w-full p-4 flex gap-2">
                <Input type="email" variant="bordered" className="text-black" placeholder="cari promo ..." startContent={
            ...
            <Modal isOpen={isOpenAdd} onOpenChange={onOpenChangeAdd}>
                <ModalContent>
                    {(onCloseAdd) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1 text-black">Tambah Promo</ModalHeader>
                            <ModalBody>
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="" className="text-black">Nama Promo</label>
                                    <Input type="text" variant="bordered" className="text-black" placeholder="Promo 11.11" value={name}
                                        onChange={(e) => setName(e.target.value)} />
                                </div>
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="" className="text-black">Diskon Promo</label>
                                    <Input type="number" variant="bordered" className="text-black" placeholder="20" value={discount.toString()}
            ...
            <Modal isOpen={isOpenEdit} onOpenChange={onOpenChangeEdit}>
                <ModalContent>
                    {(onCloseEdit) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1 text-black">Ubah Promo</ModalHeader>
                            <ModalBody>
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="" className="text-black">Nama Promo</label>
                                    <Input type="text" variant="bordered" className="text-black" placeholder="Promo 11.11" value={name}
                            ...
                            <ModalFooter>
                                <div className="flex gap-2 w-full">
                                    ...
                                    <button className="bg-black rounded-lg p-3 w-full" onClick={editPromo}>Ubah</button>
                                </div>
                            </ModalFooter>
                        </>
                    )}
            ...
            <Modal isOpen={isOpenDelete} onOpenChange={onOpenChangeDelete}>
                <ModalContent>
                    {(onCloseDelete) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1 text-black">Hapus Promo</ModalHeader>
                            <ModalBody>
                                <span className="text-black">Apakah anda yakin ingin menghapus Promo ini ? </span>
                            </ModalBody>
                            <ModalFooter>
                                <div className="flex gap-2 w-full">
