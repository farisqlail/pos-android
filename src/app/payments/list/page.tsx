...
const PaymentListPage = () => {
    const [paymentItems, setPaymentItem] = useState<PaymentItem[]>([]);
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                const data = await getResource<{ data: PaymentItem[] }>("payments");
                setPaymentItem(data.data);
                setLoading(false);
            } catch (error) {
    ...
    const addPayment = async () => {
        ...
        try {
            const data = await postResource("payments", dataPayment);
            if (data) {
                onCloseAdd()
                location.reload();
            } else {
    ...
    const openModalEdit = (item: PaymentItem) => {
    ...
    const editPayment = async () => {
        ...
        try {
            const data = await updateResource(`payments/${idPayment}`, dataPayment);
            if (data) {
                onCloseEdit()
                location.reload();
            } else {
    ...
    const deleteData = async () => {
        try {
            await deleteResource(`payments/${idPayment}`);
            onCloseDelete();
            location.reload();
        } catch (error) {
            console.log("Error fetching menus:", error);
    ...
    return (
        <div className="min-h-screen bg-[#f2f2f2] flex flex-col gap-1 text-black">
            ...
            <div className="w-full p-4 flex gap-2">
                <Input type="email" variant="bordered" className="text-black" placeholder="cari pembayaran ..." startContent={
            ...
            <Modal isOpen={isOpenAdd} onOpenChange={onOpenChangeAdd}>
                <ModalContent>
                    {(onCloseAdd) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1 text-black">Tambah Pembayaran</ModalHeader>
                            <ModalBody>
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="" className="text-black">Nama Pembayaran</label>
                                    <Input type="text" variant="bordered" className="text-black" placeholder="QRIS" value={name}
            ...
            <Modal isOpen={isOpenEdit} onOpenChange={onOpenChangeEdit}>
                <ModalContent>
                    {(onCloseEdit) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1 text-black">Ubah Pembayaran</ModalHeader>
                            <ModalBody>
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="" className="text-black">Nama Pembayaran</label>
                                    <Input type="text" variant="bordered" className="text-black" placeholder="Qris" value={name}
                            ...
                            <ModalFooter>
                                <div className="flex gap-2 w-full">
                                    ...
                                    <button className="bg-black rounded-lg p-3 w-full" onClick={editPayment}>Ubah</button>
                                </div>
                            </ModalFooter>
                        </>
                    )}
            ...
            <Modal isOpen={isOpenDelete} onOpenChange={onOpenChangeDelete}>
                <ModalContent>
                    {(onCloseDelete) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1 text-black">Hapus Pembayaran</ModalHeader>
                            <ModalBody>
                                <span className="text-black">Apakah anda yakin ingin menghapus pembayaran ini ? </span>
                            </ModalBody>
                            <ModalFooter>
                                <div className="flex gap-2 w-full">
