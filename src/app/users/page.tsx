...
const UsersPage = () => {
    const [usersItems, setUsersItem] = useState<UsersItem[]>([]);
    ...
    const filteredUserItems = usersItems.filter(item =>
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                const data = await getResource<{ data: UsersItem[] }>("users");
    ...
    const addUser = async () => {
        ...
        try {
            const data = await postResource("users", dataUser);
            if (data) {
                onCloseAdd()
                location.reload();
            } else {
    ...
    const editUser = async () => {
        ...
        try {
            const data = await updateResource(`users/${idUser}`, dataUser);
            if (data) {
                onCloseEdit()
                location.reload();
            } else {
    ...
    const deleteData = async () => {
        try {
            await deleteResource(`users/${idUser}`);
            onCloseDelete();
            location.reload();
        } catch (error) {
            console.log("Error fetching menus:", error);
    ...
    return (
        <div className="min-h-screen bg-[#f2f2f2] flex flex-col gap-1 text-black">
            ...
            <div className="w-full p-4 flex gap-2">
                <Input type="email" variant="bordered" className="text-black" placeholder="cari pengguna ..." startContent={
            ...
            <div className="flex flex-col gap-4 pr-4 pl-4 mt-2">
                {loading ? (
                    ...
                    filteredUserItems.map((user) => (
                        <div key={user.id} className="p-4 border border-black rounded-lg flex justify-between items-center">
                            <span className="font-semibold">{user.name} | {user.role}</span>
                            <div className="flex gap-2">
                                <div className="border rounded-full p-1 border-black" onClick={() => openModalEdit(user)}>
                                ...
                                <div className="border rounded-full p-1 border-[#e60000]" onClick={() => openDelete(user.id)}>
            ...
            <Modal isOpen={isOpenAdd} onOpenChange={onOpenChangeAdd}>
                <ModalContent>
                    {(onCloseAdd) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1 text-black">Tambah Pengguna</ModalHeader>
                            <ModalBody>
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="" className="text-black">Nama Lengkap</label>
                                    <Input type="text" variant="bordered" className="text-black" placeholder="John Doe" value={name}
                                        onChange={(e) => setName(e.target.value)} />
                                </div>
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="" className="text-black">Email</label>
                                    <Input type="email" variant="bordered" className="text-black" placeholder="Johndoe@gmail.com" value={email}
                                        onChange={(e) => setEmail(e.target.value)} />
                                </div>
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="" className="text-black">Role</label>
                                    <Select
                                        label="Pilih Role"
                                        value={role}
                                    ...
                                    >
                                        <SelectItem key="owner" value="owner" className="text-black">Owner</SelectItem>
                                        <SelectItem key="cashier" value="cashier" className="text-black">Cashier</SelectItem>
                                        <SelectItem key="staff" value="staff" className="text-black">Staff</SelectItem>
                                    </Select>
                                </div>
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="" className="text-black">Password</label>
                                    <Input type="password" variant="bordered" className="text-black" placeholder="password" value={password}
                            ...
                            <ModalFooter>
                                <div className="flex gap-2 w-full">
                                    ...
                                    <button className="bg-black rounded-lg p-3 w-full" onClick={() => {
                                        addUser();
                                        onCloseAdd();
                                    }}>Tambah</button>
                                </div>
            ...
            <Modal isOpen={isOpenEdit} onOpenChange={onOpenChangeEdit}>
                <ModalContent>
                    {(onCloseEdit) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1 text-black">Ubah Pengguna</ModalHeader>
                            <ModalBody>
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="" className="text-black">Nama Lengkap</label>
                                    <Input type="text" variant="bordered" className="text-black" placeholder="John Doe" value={name}
                            ...
                            <ModalFooter>
                                <div className="flex gap-2 w-full">
                                    ...
                                    <button className="bg-black rounded-lg p-3 w-full" onClick={editUser}>Ubah</button>
                                </div>
                            </ModalFooter>
                        </>
                    )}
            ...
            <Modal isOpen={isOpenDelete} onOpenChange={onOpenChangeDelete}>
                <ModalContent>
                    {(onCloseDelete) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1 text-black">Hapus Pengguna</ModalHeader>
                            <ModalBody>
                                <span className="text-black">Apakah anda yakin ingin menghapus pengguna ini ? </span>
                            </ModalBody>
                            <ModalFooter>
                                <div className="flex gap-2 w-full">
